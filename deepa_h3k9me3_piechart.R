...
res=read.table(pipe("more H3_down.annStats |grep -v '0.0'|cut -f1,2,4"), sep="\t",header=F)
...
res=read.table(pipe("more H3_up.annStats |grep -v '0.0'|cut -f1,2,4"), sep="\t",header=F)
i1 = which(res[,1]=="Annotation")[2]+1
i2 = dim(res)[1]
res = res[ i1:i2,]
tdown = as.numeric(as.character(res[,2]))
...
res=read.table(pipe("more k9me3_down.annStats |grep -v '0.0'|cut -f1,2,4"), sep="\t",header=F)
i1 = which(res[,1]=="Annotation")[2]+1
i2 = dim(res)[1]
res = res[ i1:i2,]
tdown = as.numeric(as.character(res[,2]))
...
res=read.table(pipe("more k9me3_up.annStats |grep -v '0.0'|cut -f1,2,4"), sep="\t",header=F)
i1 = which(res[,1]=="Annotation")[2]+1
i2 = dim(res)[1]
res = res[ i1:i2,]
tdown = as.numeric(as.character(res[,2]))
